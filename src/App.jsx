...
export default function App() {
  ...
  const catalogues = [
    { title: "Pavimento Laminado - Finsa", pdfUrl: "https://www.finsa.com/documents/d/guest/catalogo_finfloor_original_2026_dig_pt-pdf", image: catalogImagesObj["Catalogo_Finfloor_Original.png"] || fallbackImg },
    { title: "Vinílico - Tarkett", pdfUrl: "https://media.tarkett-image.com/docs/Tarkett_iD_Click_Ultimate_Brochura_Digital_PT.pdf", image: catalogImagesObj["Vinilico.png"] || fallbackImg },
    { title: "Portas Segurança - Portrisa", pdfUrl: "https://portrisa.com/assets/docs/catalogs/PTS.CAT.PS.PT.005_Catalogo%20Portas%20Seguranca_Portrisa.pt.pdf", image: catalogImagesObj["Porta_Seguranca.png"] || fallbackImg },
    { title: "Portas Corta Fogo - Portrisa", pdfUrl: "https://portrisa.com/assets/docs/catalogs/PTS.CAT.PCF.PTES.005_%20Portas%20Corta%20Fogo_Portrisa.pt.es.pdf", image: catalogImagesObj["Porta_CortaFogo.png"] || fallbackImg },
    { title: "Portas Interiores - MCL", pdfUrl: "https://media.mcl.pt//multimedia/DOCUMENTOS/45/CATALOGO_PORBLOCK_PORTAX%20-%20Vs1.2.pdf", image: catalogImagesObj["Porta_Interior.png"] || fallbackImg },
    { title: "Cabines WC e Revestimentos Fenólicos - MCL", pdfUrl: "https://media.mcl.pt//multimedia/DOCUMENTOS/1710/MCL_CatalogoProjeto_digital_compressed.pdf", image: catalogImagesObj["Fenolico.png"] || fallbackImg },
    { title: "Cozinhas", pdfUrl: "https://media.mcl.pt//multimedia/DOCUMENTOS/47/CATALOGO_GERAL_COMPONENTES%20-%20Vs1.34.pdf", image: catalogImagesObj["Cozinha.png"] || fallbackImg },
    { title: "Tetos e Revestimentos PVC - Sagiper", pdfUrl: "https://sagiper.com/wp-content/uploads/2016/08/20250909-SAGIREV.pdf", image: catalogImagesObj["PVC.png"] || fallbackImg },
  ];
  ...
  useEffect(() => {
    const handleKeyDown = (e) => { if (e.key === 'Escape') setActiveLightbox(null); };
  ...
  const handleNextHero = () => { setCurrentHeroIdx((prev) => (prev + 1) % heroImages.length); startSliderTimer(); };
  const handlePrevHero = () => { setCurrentHeroIdx((prev) => (prev - 1 + heroImages.length) % heroImages.length); startSliderTimer(); };
  const scrollToTop = () => { window.scrollTo({ top: 0, behavior: 'smooth' }); };

  const nextServiceImg = (serviceIdx, e) => {
    e.stopPropagation();
    setServices(prev => prev.map((s, idx) => idx === serviceIdx ? { ...s, currentIdx: (s.currentIdx + 1) % s.images.length } : s));
  };
  ...
  return (
    <HelmetProvider>
      ...
      <div 
        className="min-h-screen font-sans text-stone-800 antialiased selection:bg-[#5D4A3A]/30 selection:text-stone-900 overflow-x-hidden w-full relative"
        style={{
      ...
      <div className={`fixed inset-0 z-40 bg-[#5D4A3A]/98 backdrop-blur-lg transition-all duration-300 flex flex-col items-center justify-center space-y-8 text-white ${mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <a href="#services" onClick={() => setMobileMenuOpen(false)} className="text-2xl font-semibold hover:text-[#e3be98] transition-colors">Serviços</a>
      ...
      <header className="relative h-screen w-full bg-stone-900 text-white overflow-hidden flex items-center">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8 text-center md:text-left z-10 w-full mt-20 flex flex-col items-center md:items-start">
          ...
          <div className="mt-8 flex flex-col justify-center gap-4 sm:flex-row md:justify-start w-full sm:w-auto px-4 sm:px-0">
            <a href={`mailto:${businessEmail}?subject=Pedido de Orçamento`} className="inline-flex items-center justify-center rounded-xl bg-[#b5895a] px-8 py-3.5 text-sm font-semibold tracking-wider text-white shadow-xl hover:bg-[#5D4A3A] transition duration-300 transform hover:scale-[1.02]">
        ...
        {heroImages.length > 1 && (
          <>
            <button onClick={handlePrevHero} className="absolute left-3 sm:left-6 top-1/2 -translate-y-1/2 p-2.5 sm:p-3 rounded-xl bg-[#b5895a]/90 text-white hover:bg-[#5D4A3A] transition-all duration-300 z-20 shadow-xl backdrop-blur-xs block" aria-label="Anterior">
              <ChevronLeft className="h-6 w-6 sm:h-7 sm:w-7" />
            </button>
            <button onClick={handleNextHero} className="absolute right-3 sm:right-6 top-1/2 -translate-y-1/2 p-2.5 sm:p-3 rounded-xl bg-[#b5895a]/90 text-white hover:bg-[#5D4A3A] transition-all duration-300 z-20 shadow-xl backdrop-blur-xs block" aria-label="Seguinte">
      ...
      {/* SECÇÃO: SERVIÇOS */}
      <section id="services" className="mx-auto max-w-full px-4 sm:max-w-[85vw] sm:px-6 py-20 sm:py-28 scroll-mt-12 overflow-hidden">
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 w-full">
          {services.map((service, serviceIdx) => (
            <div key={serviceIdx} className="flex flex-col bg-[#faf6f0]/90 rounded-3xl p-4 border border-stone-300/40 shadow-xs hover:shadow-md transition-all duration-300 group overflow-hidden w-full">
              <div 
              ...
              >
                ...
                {service.images.length > 1 && (
                  <div className="absolute inset-x-2 top-1/2 -translate-y-1/2 flex justify-between opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300 z-10">
                    <button onClick={(e) => prevServiceImg(serviceIdx, e)} className="bg-black/40 sm:bg-white/80 backdrop-blur-xs text-white sm:text-stone-800 p-2 sm:p-1.5 rounded-xl hover:bg-white hover:text-stone-900 transition shadow-sm" aria-label="Anterior">
                      <ChevronLeft className="h-4 w-4" />
                    </button>
                    <button onClick={(e) => nextServiceImg(serviceIdx, e)} className="bg-black/40 sm:bg-white/80 backdrop-blur-xs text-white sm:text-stone-800 p-2 sm:p-1.5 rounded-xl hover:bg-white hover:text-stone-900 transition shadow-sm" aria-label="Seguinte">
                ...
                {service.images.length > 1 && (
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex space-x-1 bg-black/40 px-2 py-1 rounded-full backdrop-blur-xs z-10">
      ...
      {/* COMPROMISSOS */}
      <section className="bg-[#5D4A3A] text-[#f4f1ea] py-16 sm:py-24 rounded-t-[2.5rem] sm:rounded-t-[3rem] shadow-2xl relative overflow-hidden w-full">
        <div className="mx-auto max-w-7xl px-6 lg:px-8 grid gap-10 grid-cols-1 sm:grid-cols-2 md:grid-cols-5 relative z-10">
          <div className="flex flex-col items-center text-center md:items-start md:text-left">
            <div className="p-3 bg-[#4d3d30] rounded-xl mb-4 text-[#b5895a]"><Award className="h-6 w-6" /></div>
          ...
          <div className="flex flex-col items-center text-center md:items-start md:text-left">
            <div className="p-3 bg-[#4d3d30] rounded-xl mb-4 text-[#b5895a]"><Clock className="h-6 w-6" /></div>
          ...
          <div className="flex flex-col items-center text-center md:items-start md:text-left">
            <div className="p-3 bg-[#4d3d30] rounded-xl mb-4 text-[#b5895a]"><Hammer className="h-6 w-6" /></div>
          ...
          <div className="flex flex-col items-center text-center md:items-start md:text-left">
            <div className="p-3 bg-[#4d3d30] rounded-xl mb-4 text-[#b5895a]"><Shield className="h-6 w-6" /></div>
          ...
          <div className="flex flex-col items-center text-center md:items-start md:text-left">
            <div className="p-3 bg-[#4d3d30] rounded-xl mb-4 text-[#b5895a]"><Star className="h-6 w-6" /></div>
      ...
      {/* SECÇÃO: PARCEIROS */}
      <section id="partners" className="bg-[#ebd9c5]/20 border-y border-stone-300/40 py-16 sm:py-20 scroll-mt-12 overflow-hidden w-full">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-6 sm:grid-cols-3 md:grid-cols-4 items-center justify-items-center">
      ...
      {/* SECÇÃO: CATÁLOGOS */}
      <section id="catalogues" className="mx-auto max-w-7xl px-4 sm:px-6 pt-20 sm:pt-28 pb-12 lg:px-8 scroll-mt-12 overflow-hidden w-full">
        <div className="text-center max-w-3xl mx-auto mb-16">
      ...
      {/* SECÇÃO: CONTACTOS */}
      <section id="contact" className="mx-auto max-w-4xl px-4 sm:px-6 pt-12 pb-28 text-center lg:px-8 scroll-mt-12 overflow-hidden w-full">
        ...
        <div className="mt-12 grid gap-4 grid-cols-1 sm:grid-cols-2">
          <a href={`tel:${businessPhone}`} className="flex items-center p-4 sm:p-5 rounded-2xl bg-[#faf6f0]/90 border border-stone-300/50 hover:border-[#b5895a] hover:shadow-md transition-all duration-300 text-left group">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base font-bold text-stone-900 mt-0.5">{businessPhone}</p>
            </div>
          </a>
          
          <a href={`mailto:${businessEmail}`} className="flex items-center p-4 sm:p-5 rounded-2xl bg-[#faf6f0]/90 border border-stone-300/50 hover:border-[#5D4A3A] hover:shadow-md transition-all duration-300 text-left group">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-xs sm:text-base font-bold text-stone-900 mt-0.5 truncate">{businessEmail}</p>
            </div>
          </a>

          <a href={`https://wa.me/${businessPhone}`} target="_blank" rel="noreferrer" className="flex items-center p-4 sm:p-5 rounded-2xl bg-[#faf6f0]/90 border border-stone-300/50 hover:border-[#25D366] hover:shadow-md transition-all duration-300 text-left group">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base font-bold text-stone-900 mt-0.5 group-hover:text-[#25D366] transition-colors">WhatsApp</p>
            </div>
          </a>

          <a href={`${facebookUser}`} target="_blank" rel="noreferrer" className="flex items-center p-4 sm:p-5 rounded-2xl bg-[#faf6f0]/90 border border-stone-300/50 hover:border-[#1877F2] hover:shadow-md transition-all duration-300 text-left group">
            ...
            <div>
              ...
              <p className="text-sm sm:text-base font-bold text-stone-900 mt-0.5 group-hover:text-[#1877F2] transition-colors">Facebook</p>
            </div>
          </a>
        </div>
      </section>
